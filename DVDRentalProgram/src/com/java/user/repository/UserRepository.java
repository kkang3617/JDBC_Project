...
import com.java.user.domain.User; //CTRL + shift + o 자동임포트
...
public class UserRepository {
	...
	public void addUser(User user) {
		...
		/*
		 ...
		 - try문에 자원 객체를 전달하면, try가 끝날 경우
		   자동으로 자원을 종효해주는 기능. (finally에서 따로 close() 안해도됨)
		 - 자원을 종료하려는 객체가 AutoCloseable 인터페이스의 구현체여 한다.
		 */
		try(Connection conn = connection.getConnetion();
	...
	public List<User> findByUserName(String userName) {
		...
		try(Connection conn = connection.getConnetion();
			...
			while(rs.next()) { //rs.next()조회결과 있다면 true, 없다면 false
